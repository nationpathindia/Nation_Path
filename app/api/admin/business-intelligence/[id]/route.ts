//////////////////////////////////////////////////////////////
// NATIONPATH ASTRO BUSINESS INTELLIGENCE CMS ADMIN API
//
// GET    -> Single Business Intelligence
// PUT    -> Update Business Intelligence
// DELETE -> Delete Business Intelligence
//
// Responsibility:
// Astrology knowledge management only.
//
// Does NOT:
// - calculate astrology
// - modify prediction engine
// - modify Swiss Ephemeris
//////////////////////////////////////////////////////////////

import { NextRequest, NextResponse } from "next/server";

import { connectMongoDB } from "@/lib/mongodb";

import BusinessIntelligence from "@/app/models/BusinessIntelligence";





export const dynamic = "force-dynamic";









//////////////////////////////////////////////////////////////
// GET SINGLE BUSINESS INTELLIGENCE
//////////////////////////////////////////////////////////////

export async function GET(

 req:NextRequest,

 context:any

){


  try{


    await connectMongoDB();




    const { id } = context.params;






    const data =


      await (BusinessIntelligence as any)

      .findById(id)

      .lean();






    if(!data){


      return NextResponse.json(

        {

          success:false,

          message:

          "Business intelligence not found",

        },

        {

          status:404,

        }

      );


    }







    return NextResponse.json(

      {

        success:true,

        data,

      }

    );



  }


  catch(error:any){


    console.error(

      "[BUSINESS_INTELLIGENCE_SINGLE_GET_ERROR]",

      error

    );






    return NextResponse.json(

      {

        success:false,

        message:

        "Failed to fetch business intelligence",

        error:

        error.message,

      },

      {

        status:500,

      }

    );


  }


}









//////////////////////////////////////////////////////////////
// UPDATE BUSINESS INTELLIGENCE
//////////////////////////////////////////////////////////////

export async function PUT(

 req:NextRequest,

 context:any

){


  try{


    await connectMongoDB();




    const { id } = context.params;






    const body =


      await req.json();







    if(body.slug){


      body.slug =


        String(body.slug)

        .trim()

        .toLowerCase();


    }








    const updated =


      await (BusinessIntelligence as any)

      .findByIdAndUpdate(

        id,

        body,

        {

          new:true,

        }

      );








    if(!updated){


      return NextResponse.json(

        {

          success:false,

          message:

          "Business intelligence not found",

        },

        {

          status:404,

        }

      );


    }








    return NextResponse.json(

      {

        success:true,

        message:

        "Business intelligence updated successfully",

        data:updated,

      }

    );





  }


  catch(error:any){


    console.error(

      "[BUSINESS_INTELLIGENCE_UPDATE_ERROR]",

      error

    );








    if(error.code === 11000){


      return NextResponse.json(

        {

          success:false,

          message:

          "Duplicate slug",

        },

        {

          status:409,

        }

      );


    }







    return NextResponse.json(

      {

        success:false,

        message:

        "Failed to update business intelligence",

        error:

        error.message,

      },

      {

        status:500,

      }

    );


  }


}









//////////////////////////////////////////////////////////////
// DELETE BUSINESS INTELLIGENCE
//////////////////////////////////////////////////////////////

export async function DELETE(

 req:NextRequest,

 context:any

){


  try{


    await connectMongoDB();




    const { id } = context.params;







    const deleted =


      await (BusinessIntelligence as any)

      .findByIdAndDelete(id);








    if(!deleted){


      return NextResponse.json(

        {

          success:false,

          message:

          "Business intelligence not found",

        },

        {

          status:404,

        }

      );


    }








    return NextResponse.json(

      {

        success:true,

        message:

        "Business intelligence deleted successfully",

      }

    );





  }


  catch(error:any){


    console.error(

      "[BUSINESS_INTELLIGENCE_DELETE_ERROR]",

      error

    );







    return NextResponse.json(

      {

        success:false,

        message:

        "Failed to delete business intelligence",

        error:

        error.message,

      },

      {

        status:500,

      }

    );


  }


}