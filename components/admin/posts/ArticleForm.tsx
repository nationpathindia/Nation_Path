...
const AI_MODULES = [
  {
    id: "mainStory",
    title: "Main Story",
    desc: "Deep analysis with integrated official statements",
    default: true,
  },
  {
    id: "background",
    title: "Background",
    desc: "Historical context & previous events",
    default: true,
  },
  {
    id: "expertOpinion",
    title: "Expert Opinion",
    desc: "Analysis from experts & analysts",
    default: true,
  },
  {
    id: "brief",
    title: "Brief",
    desc: "Sharp 80-word summary",
    default: true,
  },
  {
    id: "keyTakeaways",
    title: "Key Takeaways",
    desc: "3-5 data-driven bullet points",
    default: true,
  },
  {
    id: "keyHighlights",
    title: "Key Highlights",
    desc: "Structured: Issue, Location, Authority, Action, Impact",
    default: true,
  },
  {
    id: "whyItMatters",
    title: "Why It Matters",
    desc: "Broader impact & objective analysis",
    default: true,
  },
  {
    id: "whatsNext",
    title: "What's Next",
    desc: "Upcoming developments & timelines",
    default: true,
  },
  {
    id: "timeline",
    title: "Timeline",
    desc: "Historical context & chronological events",
    default: false,
  },
  {
    id: "factChecks",
    title: "Fact-Check",
    desc: "Verify claims & allegations",
    default: false,
  },
  {
    id: "faqs",
    title: "FAQs",
    desc: "3-5 practical reader questions",
    default: true,
  },
  {
    id: "suggestedCategory",
    title: "Auto Category",
    desc: "AI-suggested category & sub-category",
    default: true,
  },
  {
    id: "seo",
    title: "SEO Metadata",
    desc: "Title, Description, Keywords, Slug",
    default: true,
  },
];
...
export default function ArticleForm({
  ...
  const typeFromUrl = (postType ||
    searchParams.get("type") ||
    "news") as "news" | "editorial";
  ...
  function handleOpenModuleSelection() {
    if (!form.title?.trim()) {
      setLocalError(
        "Please enter a Title first so AI can understand the context."
      );
      return;
    }

    const textToAnalyze =
      form.content || form.shortBrief || "";

    if (!textToAnalyze.trim()) {
      setLocalError(
        "Please add some content or notes for the AI to analyze."
      );
      return;
    }
  ...
  async function handleGenerateAIIntelligence() {
    ...
    const modulesToGenerate = Object.entries(
      selectedModules
    )
    ...
    try {
      const response = await fetch(
        "/api/ai/news/generate-intelligence",
        {
          method: "POST",
          headers: {
  ...
  function handleProcessForDraft() {
    ...
    const highlightsArray =
      aiPreviewData.keyHighlights
        ? [
            `Issue: ${
              aiPreviewData.keyHighlights.issue ||
              "N/A"
            }`,
            `Location: ${
              aiPreviewData.keyHighlights.location ||
              "N/A"
            }`,
            `Authority: ${
              aiPreviewData.keyHighlights.authority ||
              "N/A"
            }`,
            `Action: ${
              aiPreviewData.keyHighlights.actionTaken ||
              "N/A"
            }`,
            `Impact: ${
              aiPreviewData.keyHighlights.impact ||
              "N/A"
            }`,
          ]
        : [];
    ...
    const expertOpinionString =
      aiPreviewData.expertOpinion?.length > 0
        ? aiPreviewData.expertOpinion
            .map(
              (exp: any) =>
                `${exp.expert || "Expert"} (${
                  exp.designation || "Analyst"
                }): "${exp.quote}"`
            )
            .join("\n\n")
        : "";

    // PostFormData uses categoryId, not category.
    let matchedCategoryId = form.categoryId;

    if (
      aiPreviewData.suggestedCategory &&
      categories.length > 0
    ) {
      ...
      const matchedCat = categories.find((c) => {
        ...
        const categorySlug = c.slug
          ?.toLowerCase()
          .trim();

        const suggestedSlug = suggestedCategory.replace(
          /\s+/g,
          "-"
        );
    ...
    setForm((prev) => ({
      ...
      // PostFormData field is categoryId.
      ...
      metaDescription:
        aiPreviewData.metaDescription ||
        prev.metaDescription,

      metaKeywords:
        aiPreviewData.metaKeywords ||
        prev.metaKeywords,

      slug: aiPreviewData.urlSlug
        ? aiPreviewData.urlSlug
            .toLowerCase()
            .trim()
            .replace(/[^a-z0-9]+/g, "-")
            .replace(/^-+|-+$/g, "")
      ...
      timeline:
        aiPreviewData.timeline ||
        prev.timeline,

      factCheck:
        aiPreviewData.factChecks ||
        prev.factCheck,

      whatsNext:
        aiPreviewData.whatsNext ||
        prev.whatsNext,
    ...
    setAutosaveMessage(
      "✨ AI data committed to form. Saving draft..."
    );
  ...
  async function handleSubmit(
    ...
    if (autosaveCreatingRef.current) {
      setLocalError(
        "Draft is being saved. Please wait..."
      );
      return;
    }
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-400 mt-1">
                  Generate modular intelligence with
                  ...
                  <div className="flex flex-col items-center justify-center h-full w-full py-16 space-y-8 bg-[#0a0f1e]">
                    ...
                    <div className="text-center space-y-2">
                      <h3 className="text-xl font-bold text-white">
                        AI is crafting your article...
                      </h3>

                      <p className="text-sm text-gray-400 max-w-md">
                        Analyzing context, drafting
                        sections, and optimizing SEO.
                        Please wait a few seconds.
                      </p>
                    </div>

                    <div className="w-full max-w-md space-y-3 px-4">
                      <div className="h-2 bg-gray-700/50 rounded-full animate-pulse w-3/4 mx-auto" />
                      <div className="h-2 bg-gray-700/50 rounded-full animate-pulse w-1/2 mx-auto opacity-75" />
                      <div className="h-2 bg-gray-700/50 rounded-full animate-pulse w-5/6 mx-auto opacity-50" />
                    </div>
                  </div>
                ) : (
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-8 scroll-smooth bg-[#0a0f1e]">
                    {aiPreviewData.mainStory && (
                      ...
                      >
                        ...
                        <div
                          className="bg-white/5 rounded-xl p-5 text-sm leading-relaxed text-gray-200 border border-gray-800"
                    ...
                    {aiPreviewData.brief && (
                      ...
                      >
                        ...
                        <p className="bg-white/5 rounded-xl p-5 text-sm text-gray-200 border border-gray-800">
                          {aiPreviewData.brief}
                        </p>
                      </div>
                    )}
                    ...
                    {aiPreviewData.keyTakeaways
                      ...
                      >
                        ...
                        <ul className="bg-white/5 rounded-xl p-5 space-y-3 border border-gray-800">
                          {aiPreviewData.keyTakeaways.map(
                            (
                              takeaway: string,
                              idx: number
                            ) => (
                              <li
                                key={idx}
                                className="text-sm flex items-start gap-3 text-gray-200"
                              >
                    ...
                    {aiPreviewData.background && (
                      ...
                      >
                        ...
                        <div className="bg-white/5 rounded-xl p-5 text-sm leading-relaxed text-gray-200 border border-gray-800">
                          {
                            aiPreviewData.background
                          }
                        </div>
                    ...
                    {aiPreviewData.expertOpinion
                      ...
                      >
                        ...
                        <div className="space-y-3">
                          {aiPreviewData.expertOpinion.map(
                            (
                              exp: any,
                              idx: number
                            ) => (
                              <div
                                key={idx}
                                className="bg-white/5 rounded-xl p-5 border-l-4 border-orange-500 border border-gray-800"
                              >
                        ...
                        <div className="bg-white/5 rounded-xl p-5 space-y-4 border border-gray-800">
                          {aiPreviewData.timeline.map(
                            (
                              ...
                              <div
                                key={idx}
                                className="flex gap-4 border-b border-gray-700 pb-3 last:border-0 last:pb-0"
                              >
                                <div className="text-orange-400 font-bold text-sm w-24 flex-shrink-0">
                                  {item.year}
                                </div>
                                ...
                                <div className="flex-1">
                                  ...
                                  <div className="text-xs text-orange-400 mt-1 uppercase">
                                    {item.status}
                                  </div>
                                </div>
                              </div>
                        ...
                        <div className="bg-white/5 rounded-xl p-5 space-y-4 border border-gray-800">
                          {aiPreviewData.faqs.map(
                            (
                              ...
                              <div
                                key={idx}
                                className="pb-3 border-b border-gray-700 last:border-0 last:pb-0"
                              >
                                <div className="font-bold text-sm text-white">
                                  Q:{" "}
                                  {
                                ...
                                <div className="text-sm text-gray-300 mt-2 pl-3 border-l-2 border-gray-600">
                                  A:{" "}
                                  {faq.answer}
                                </div>
                              </div>
                    ...
                    {aiPreviewData.seoTitle && (
                      ...
                      >
                        ...
                        <div className="bg-white/5 rounded-xl p-5 space-y-3 text-sm border border-gray-800">
                          <div>
                            <span className="text-gray-400 font-semibold w-24 inline-block">
                              Title:
                            </span>{" "}
                            <span className="text-white">
                              {
              ...
              {!aiPreviewData ? (
                <>
                  <button
                    ...
                    className="px-5 py-2.5 rounded-lg border border-gray-700 hover:bg-white/5 transition text-gray-300"
                  >
                    Cancel
                  </button>
                  ...
                  <button
                    ...
                    className="px-5 py-2.5 rounded-lg bg-orange-600 hover:bg-orange-700 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 text-white font-semibold"
                  >
                    {isGeneratingAI ? (
                      <>
                        <svg
                ...
                <>
                  ...
                  <button
                    onClick={
                      handleProcessForDraft
                    }
                    className="px-6 py-2.5 rounded-lg bg-orange-600 hover:bg-orange-700 transition font-bold text-white shadow-lg shadow-orange-600/20 flex items-center gap-2"
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
